...
1. **Pushes & Pull Requests to `main`**: Compiles the LaTeX document and uploads the compiled PDF as a GitHub workflow run artifact (`resume-pdf`) so it can be previewed/downloaded directly from the run page.
2. **GitHub Release Publication**: When a release is published, the workflow compiles the resume and automatically attaches `resume.pdf` to the release assets using the GitHub CLI (`gh release upload`).
3. **Manual Trigger (`workflow_dispatch`)**: Allows compiling the PDF on-demand from the GitHub Actions tab.
...
6. The GitHub Action will trigger, compile the PDF, and attach the compiled `resume.pdf` to the release assets.
